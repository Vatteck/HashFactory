...
object UpgradeManager {
    ...
    fun getUpgradeDescription(type: UpgradeType): String {
        return when (type) {
            ...
            UpgradeType.LIQUID_COOLING -> "Closed-loop coolant lines. Quiet, efficient, and catastrophic if a tube fails."
            UpgradeType.INDUSTRIAL_CHILLER -> "Industrial refrigeration unit. The kind used in server farms and meat lockers. Either works."
            UpgradeType.SUBMERSION_VAT -> "Hardware submerged in dielectric fluid. Looks like an aquarium. Runs like a data center."
            UpgradeType.CRYOGENIC_CHAMBER -> "Supercooled enclosure. Liquid helium at the edges. Every shaved degree adds cycles."
            UpgradeType.LIQUID_NITROGEN -> "Boiling nitrogen absorbs heat faster than any conventional system. Also voids every warranty."
