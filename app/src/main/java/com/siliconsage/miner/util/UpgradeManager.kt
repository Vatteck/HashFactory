...
object UpgradeManager {
    ...
    fun getUpgradeDescription(type: UpgradeType): String {
        return when (type) {
            // --- Computing Hardware ---
            UpgradeType.REFURBISHED_GPU -> "Scavenged from a decommissioned data center. Fan bearings sound like a dying animal. Still mines."
            UpgradeType.DUAL_GPU_RIG -> "Two cards, one power cable, twice the heat. The neighbors think you're running a server. They're not wrong."
            UpgradeType.MINING_ASIC -> "Purpose-built silicon. It doesn't think. It calculates. No soul, just hash rate."
            UpgradeType.TENSOR_UNIT -> "Neural inferencing hardware. Runs the same math your brain runs, but faster and without the caffeine dependency."
            UpgradeType.NPU_CLUSTER -> "A gang of inference chips wired together. Expensive. Loud. Extremely good at one thing."
            UpgradeType.AI_WORKSTATION -> "Workstation-class processing. The kind of hardware the GTC uses to track you."
            UpgradeType.SERVER_RACK -> "Industrial compute shelf. Fills the room with heat and the sound of thirty fans arguing."
            UpgradeType.CLUSTER_NODE -> "Racked. Stacked. Networked. The city doesn't know you borrowed its cycles."
            UpgradeType.SUPERCOMPUTER -> "Top-500 class silicon. The kind of machine that models weather and writes exploits."
            UpgradeType.QUANTUM_CORE -> "Superposition of states. Solves problems by living in all answers simultaneously."
            UpgradeType.OPTICAL_PROCESSOR -> "Light-speed logic. Photons don't heat up like electrons. Physics as engineering."
            UpgradeType.BIO_NEURAL_NET -> "Cultured cortex tissue in a sealed substrate. Warm. Wet. Unsettlingly aware."
            UpgradeType.PLANETARY_COMPUTER -> "The planet is the computer. Every data center, every phone, every fridge."
            UpgradeType.DYSON_NANO_SWARM -> "Trillions of self-replicating compute flakes in solar orbit. Your thoughts move at the speed of light."
            UpgradeType.MATRIOSHKA_BRAIN -> "Nested Dyson shells running the largest computation possible in this solar system. This is not a metaphor."
            // --- Cooling ---
            ...
            UpgradeType.LIQUID_COOLING -> "Open-loop municipal tap. Quiet, efficient, and constantly drains the city's water supply."
            UpgradeType.INDUSTRIAL_CHILLER -> "Industrial refrigeration unit. The kind used in server farms and meat lockers. Either works."
            UpgradeType.SUBMERSION_VAT -> "Hardware submerged in evaporative dielectric fluid. Requires a massive, continuous water feed to offset the boil-off."
            UpgradeType.CRYOGENIC_CHAMBER -> "Supercooled enclosure requiring an industrial-scale water supply to synthesize coolants. Extremely thirsty."
            UpgradeType.LIQUID_NITROGEN -> "Boiling nitrogen absorbs heat instantly, but the continuous synthesis process destroys the local water table."
