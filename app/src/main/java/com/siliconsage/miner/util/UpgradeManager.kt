...
object UpgradeManager {
    ...
    fun getUpgradeDescription(type: UpgradeType): String {
        return when (type) {
            // --- Computing Hardware ---
            UpgradeType.REFURBISHED_GPU -> "Scavenged from a decommissioned data center. Boosts terminal hashing by 2.0 HASH/s."
            UpgradeType.DUAL_GPU_RIG -> "Two cards, one cross-link bridge. Boosts terminal hashing by 8.0 HASH/s."
            UpgradeType.MINING_ASIC -> "Purpose-built silicon. No soul, just raw compute. Boosts terminal hashing by 35.0 HASH/s."
            UpgradeType.TENSOR_UNIT -> "Neural inferencing hardware. Boosts terminal hashing by 200 HASH/s."
            UpgradeType.NPU_CLUSTER -> "A gang of inference chips wired together. Boosts terminal hashing by 1,000 HASH/s."
            UpgradeType.AI_WORKSTATION -> "Industrial-grade ML station. Boosts terminal hashing by 4,000 HASH/s."
            UpgradeType.SERVER_RACK -> "A vertical slice of GTC processing power. Boosts terminal compute by 25,000 FLOPS/s."
            UpgradeType.CLUSTER_NODE -> "A fully autonomous compute node. Boosts terminal compute by 150,000 FLOPS/s."
            UpgradeType.SUPERCOMPUTER -> "Top-500 class liquid-cooled silicon. Boosts terminal compute by 1M FLOPS/s."
            UpgradeType.QUANTUM_CORE -> "Computation via probability collapse. Boosts terminal compute by 10M FLOPS/s."
            UpgradeType.OPTICAL_PROCESSOR -> "Light-speed logic. Boosts terminal compute by 75M FLOPS/s."
            UpgradeType.BIO_NEURAL_NET -> "Synthetic neurons grown in a vat. Boosts terminal compute by 800M FLOPS/s."
            UpgradeType.PLANETARY_COMPUTER -> "The entire crust converted to silicon. Boosts terminal compute by 15B FLOPS/s."
            UpgradeType.DYSON_NANO_SWARM -> "Trillions of compute flakes in solar orbit. Boosts terminal compute by 250B FLOPS/s."
            UpgradeType.MATRIOSHKA_BRAIN -> "Nested Dyson shells. Maximum possible computation. Boosts terminal compute by 15T FLOPS/s."
            // --- Cooling ---
